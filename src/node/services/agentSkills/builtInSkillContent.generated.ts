...
export const BUILTIN_SKILL_FILES: Record<string, Record<string, string>> = {
  ...
  "mux-docs": {
    "references/docs/AGENTS.md": [
      ...
      "",
      "### Important directories and architecture",
      "",
      "- `src/node/`: backend services, IPC handlers, runtime/process management.",
      "- `src/browser/`: React renderer UI.",
      "- `src/common/`: shared types, model definitions, cross-layer utilities.",
      "- `src/cli/`: CLI/server entrypoints and debug tooling.",
      "- `tests/`: Jest-based unit/integration/UI tests.",
      "- `mobile/`: React Native/Expo app and its own tests.",
      "- `scripts/`: validation, CI helper, and release automation scripts.",
      "- Architecture pattern: Electron main/preload/renderer split with typed IPC; keep backend and frontend types aligned (no duplicate boundary schemas).",
      "",
      "## Documentation Rules",
      "",
      ...
      "- Format: `make fmt`",
      "- Lint: `make lint`",
      "- Test (unit default): `make test`",
      "- Test (integration): `make test-integration`",
      "- Clean: `make clean`",
      "- Dev server: `make dev` (Electron) or `make dev-server` (backend :5173 + frontend :3010)",
      "- Full local gate before push: `make static-check`",
      "",
      "### Other important scripts",
      "",
      "- `./scripts/wait_pr_ready.sh <pr_number>`: poll Codex + CI readiness together.",
      "- `./scripts/wait_pr_checks.sh <pr_number> --once`: one-shot CI checks status.",
      "- `./scripts/wait_pr_codex.sh <pr_number> --once`: one-shot Codex review status.",
      "- `./scripts/resolve_pr_comment.sh <thread_id>`: resolve review threads.",
      "",
      "## Browser Automation",
      "",
      ...
      "",
      "- Required local validation before committing: run `make static-check`; add targeted tests (`make test`, `make test-integration`, `make test-mobile`) when the touched area needs it.",
      "- Commit message convention (from recent history): prefer conventional prefixes like `feat:`, `fix:`, `docs:`, `chore:`, `refactor:`, `tests:`. Some automation-generated commits include a `🤖` prefix.",
      "- Do use clear scope in the title (`feat(remote-editor): ...` when useful); don't use vague messages like `update`.",
      "- Before creating/updating PRs or public issues, read the `pull-requests` skill and include the required attribution footer (`mux-attribution`) with model/thinking/cost values when available.",
      "- PR title convention: `🤖 <type>: <summary>` where `<type>` is typically `perf|refactor|fix|feat|ci|tests|bench`.",
      "- PR body expectation: include Summary, Background, Implementation, Validation, and Risks when relevant; keep it accurate for the **entire diff** against base.",
      "",
